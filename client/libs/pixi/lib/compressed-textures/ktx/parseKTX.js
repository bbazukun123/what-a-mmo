...
function parseKTX(arrayBuffer, supportedFormats) {
    ...
    const { littleEndian, glType, glFormat, glInternalFormat, pixelWidth, pixelHeight, numberOfMipmapLevels, offset } =
...
function getImagePixelByteSize(glType, glFormat, glInternalFormat) {
    ...
    if (glType !== 0) {
        if (_const.KTX.TYPES_TO_BYTES_PER_COMPONENT[glType]) {
            imagePixelByteSize =
                _const.KTX.TYPES_TO_BYTES_PER_COMPONENT[glType] * _const.KTX.FORMATS_TO_COMPONENTS[glFormat];
        } else {
