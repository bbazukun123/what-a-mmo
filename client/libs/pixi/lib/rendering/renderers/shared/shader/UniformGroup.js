        ...
        if (noTypes) {
            ...
            for (const i in uniformStructures) {
                ...
                if (!types.UNIFORM_TYPES_MAP[uniformData.type]) {
                    throw new Error(
                        `Uniform type ${uniformData.type} is not supported. Supported uniform types are: ${types.UNIFORM_TYPES_VALUES.join(', ')}`,
                    );
                }
                uniformData.value ??
                    (uniformData.value = getDefaultUniformValue.getDefaultUniformValue(
                        uniformData.type,
                        uniformData.size,
                    ));
    ...
    get signature() {
        {
            ...
            this._signature = createIdFromString.createIdFromString(
