...
export type ScreenId<T extends ScreenGroupId | undefined = undefined> = T extends ScreenGroupId
...
export function getScreenGroupIdByScreenId(screenId: string): ScreenGroupId | undefined {
...
for (const groupId of screenGroupIds) {
    for (const screenId of screenGroupMap[groupId]) {
        if (screenIdSet.has(screenId)) {
            throw new Error(`Duplicate screen id found: "${screenId}"! Screens must have unique ids.`);
        }
