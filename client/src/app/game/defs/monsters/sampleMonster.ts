...
export function createSampleMonsterView({ size }: { size: number }) {
...
export function stepSampleMonsterAnimation(monster: MonsterEntityType, dt: number) {
    const { monsterView } = monster.c;

    switch (monsterView.state) {
        case 'idle':
            monsterView.tick = 0;
            stepIdleAnimation(monsterView, dt);
            break;
        case 'walking':
            stepWalkingAnimation(monsterView, dt);
            break;
    }
}

function stepIdleAnimation(component: MonsterViewComponent, _: number) {
...
function stepWalkingAnimation(component: MonsterViewComponent, dt: number) {
    const { view, basePosition } = component;
    const sDt = dt * 0.001;
